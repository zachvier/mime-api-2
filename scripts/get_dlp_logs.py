...
def get_dlp_logs(token, start_date, end_date, page_size=100, next_token=None):
    """Fetches a single page of DLP logs."""
    api_url = f"{BASE_URL}/api/dlp/get-logs"
    headers = {
    ...
    try:
        ...
        print(f"Error fetching DLP logs: {e}")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        data_resp = get_dlp_logs(token, start_date, end_date, page_size, next_token)
        if not data_resp:
            break

        page_rows = []
        for block in data_resp.get("data", []):
            page_rows.extend(block.get("dlpLogs", []))
    ...
    print(f"\n--- DLP Logs ({len(all_events)} total) ---")
    if not all_events:
        print("No DLP logs found for the selected period.")
    else:

...
def get_held_release_logs(token, start_date=None, end_date=None, page_size=25, next_token=None):
    """Fetches a single page of Held/Release Message Logs."""
    api_url = f"{BASE_URL}/api/gateway/get-held-release-logs"
    headers = {
    ...
    try:
        ...
        print(f"Error fetching held/release logs: {e}")
...
if __name__ == "__main__":
    ...
    while True:
        ...
        logs_data = get_held_release_logs(token, start_date, end_date, page_size, next_token)
    ...
    print(f"\n--- Held/Release Logs Results ({len(all_logs)} total) ---")
    if not all_logs:

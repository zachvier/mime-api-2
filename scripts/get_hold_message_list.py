...
def get_hold_message_list(token, start_date=None, end_date=None, page_size=25, next_token=None, admin=True):
    """Fetches a single page of Held Messages."""
    api_url = f"{BASE_URL}/api/gateway/get-hold-message-list"
    headers = {
    ...
    try:
        ...
        print(f"Error fetching held messages: {e}")
...
if __name__ == "__main__":
    ...
    # Admin hold filter
    admin_input = input("\nShow admin held messages? (y/n, default y): ").strip().lower()
    admin = admin_input != 'n'
    ...
    while True:
        ...
        messages_data = get_hold_message_list(token, start_date, end_date, page_size, next_token, admin)
        ...
        print(f"Got {len(data)} results (total so far: {len(all_messages)})")

        meta = messages_data.get("meta", {})
    ...
    print(f"\n--- Held Messages Results ({len(all_messages)} total) ---")
    if not all_messages:
